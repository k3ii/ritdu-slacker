...
def test_no_arguments(capsys, cli):
    ...
    assert (
        out
        == """Usage: ritdu-slacker [OPTIONS] COMMAND [ARGS]...
...
def test_file(capsys, cli, filesystem, mocked_responses):
    sys.argv = [
        "ritdu-slacker",
        "file",
        "--workspace",
    ...
    filesystem.create_file("/tmp/errorlog.txt", contents="This is a test")
